nu.pi <- function(x,M=NULL,lower=0,upper=100,np=500,tol=0.1,outM=FALSE){
	...
	if (is.null(M)){ 	
		...
		if (class(y2)=="try-error" & class(y3)=="try-error" & class(y4)=="try-error" & class(y5)=="try-error"){
			...
			AMISE <- function(nu) {1/16 * (1- besselI(nu,2,expon.scaled=T)/besselI(nu,0,expon.scaled=T))^2 * integral + besselI(2*nu,0,expon.scaled=TRUE)/(2*n*pi*(besselI(nu,0,expon.scaled=TRUE))^2)}
			M<-which.min(AICs)+1
			if (length(M) == 0){
				M <- 1
				nu <- nu.rt(x)
				...
				if(integral=="NaN" | integral=="Inf"){
					...
					if (length(M)==0){
						...
						nu <- optimize(function(h) AMISE(h),interval=c(lower,upper),tol=tol)$minimum
					}
				}else{
					nu <- optimize(function(h) AMISE(h),interval=c(lower,upper),tol=tol)$minimum
				}
		...
		if (M==1){ 
			...
			if (class(y2)=="try-error"){		
				...
				if (integral=="Inf" | integral=="NaN"){
					...
					AMISE <- function(nu) {1/16 * (1- besselI(nu,2,expon.scaled=TRUE)/besselI(nu,0,expon.scaled=TRUE))^2 * integral  + besselI(2*nu,0,expon.scaled=TRUE)/(2*n*pi*(besselI(nu,0,expon.scaled=TRUE))^2)}
					nu <- optimize(function(h) AMISE(h),interval=c(lower,upper),tol=tol)$minimum
				}
			}
		}
