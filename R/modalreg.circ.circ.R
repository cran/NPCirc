...
modalreg.circ.circ<-function(x, y, t=NULL, bw=NULL, tol = 0.00001, maxit = 500, from = circular(0),
  ...
  if(is.null(bw)){
    bw <- bw.modalreg.circ.circ(x, y, lower = NULL, upper = NULL, tol = 0.00001)
  }else{
  ...
  fit <- R_modereg_CircCirc(circular(y,units="radians"),x,t,bw[1],bw[2],maxit,tol)
