ancova.circ.circ<-function(x, y, g, bw, bw1, test = "eq", method = "LL", n_boot = 500){
  ...
  if (missing(bw)){
    bw <- bw.reg.circ.circ(x, y, method = method)
  }else{
    if (is.numeric(bw)){
      if (bw < 0)
  ...
  if (missing(bw)) bw <- bw.reg.circ.circ(x, y, method = method)

  if (test == "eq"){
    m_hat <- RegCircCirc(x, y, t = x, bw = bw, method = method)
    res <- y - m_hat
  }else if (test == "paral"){
    m_hat1 <- RegCircCirc(x, y, t = x, bw = bw1, method = method)
    gamma_hat <- sapply(1:nlev, function(i) estimation_gamma(g, m_hat1, y, i))
    gamma_hats <- rep(gamma_hat, times = ni)
    m_hat <- RegCircCirc(x, y - gamma_hats, t = x, bw = bw, method = method)
  ...
  for (i in 1:nlev){
    irange <- istart:(istart + ni[i] - 1)
    mi_hat[irange] <- RegCircCirc(x[g == levels[i]], y[g == levels[i]], t = x[g == levels[i]], bw = bw, method = method)
    istart <- istart + ni[i]
  }
  ...
  for (b in 1:n_boot){
    ...
    if (test == "eq"){
      ...
      m_hat_boot <- RegCircCirc(x, y_boot, t = x, bw = bw, method = method)
    }else if (test == "paral"){
      y_boot <- (gamma_hats + m_hat + res_boot) %% (2 * pi)
      m_hat1_boot <- RegCircCirc(x, y_boot, t = x, bw = bw1, method = method)
      ...
      m_hat_boot <- RegCircCirc(x, y_boot - gamma_hats_boot, t = x, bw = bw, method = method)
    }
    ...
    for (i in 1:nlev){
      irange <- istart:(istart + ni[i] - 1)
      mi_hat_boot[irange] <- RegCircCirc(x[g == levels[i]], y_boot[g == levels[i]], t = x[g == levels[i]], bw = bw, method = method)
      istart <- istart + ni[i]
    }
  ...
  if (test == "eq"){
    meth <- "Equality test for a circular predictor and a circular response"

  }else if (test == "paral"){
    meth <- "Parallelism test for a circular predictor and a circular response"
  }
  STATISTIC <- obs
